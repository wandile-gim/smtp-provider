...
func (tq *TenantQuery) QueryConfigurations() *ConfigurationQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, tenant.ConfigurationsTable, tenant.ConfigurationsPrimaryKey...),
		)
...
func (tq *TenantQuery) Clone() *TenantQuery {
	...
	return &TenantQuery{
		...
		withConfigurations: tq.withConfigurations.Clone(),
...
func (tq *TenantQuery) WithConfigurations(opts ...func(*ConfigurationQuery)) *TenantQuery {
	query := (&ConfigurationClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withConfigurations = query
	return tq
}
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := tq.withConfigurations; query != nil {
		if err := tq.loadConfigurations(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Configurations = []*Configuration{} },
			func(n *Tenant, e *Configuration) { n.Edges.Configurations = append(n.Edges.Configurations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TenantQuery) loadConfigurations(ctx context.Context, query *ConfigurationQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Configuration)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tenant.ConfigurationsTable)
		s.Join(joinT).On(s.C(configuration.FieldID), joinT.C(tenant.ConfigurationsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tenant.ConfigurationsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tenant.ConfigurationsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Configuration](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
